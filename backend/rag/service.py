...
class RagService:
    ...
    def answer(self, doubt: str, exam_type: str | None = None, subject: str | None = None) -> dict:
        ...
        if self.config.ai_chat_url and self.config.ai_shared_token:
            reply = self._answer_with_external_ai(doubt, sources)
        elif self.config.openai_api_key and self.config.embedding_provider == "openai":
            reply = self._answer_with_openai(doubt, sources)
        else:
            reply = self._answer_without_llm(doubt, sources)
        return {"reply": reply, "sources": sources}

    def _answer_with_external_ai(self, doubt: str, sources: list[dict]) -> str:
        prompt = self._build_studybuddy_prompt(doubt, sources)
    ...
    def _answer_with_openai(self, doubt: str, sources: list[dict]) -> str:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": self._build_studybuddy_prompt(doubt, sources)},
            ],
            temperature=0.3,
        )
        return response.choices[0].message.content or ""

    def _build_studybuddy_prompt(self, doubt: str, sources: list[dict]) -> str:
        context = "\n\n".join(
        ...
        return (
            "You are StudyBuddy for JEE and NEET preparation.\n"
            "Use the retrieved question-bank context when relevant. If the context is weak, say so briefly.\n"
            "Explain the concept clearly, give a practical hint, and suggest how to practice. "
            "Do not invent exact question-bank facts that are not in the context.\n\n"
            f"Student doubt:\n{doubt}\n\n"
